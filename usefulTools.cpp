...
void usefulTools::tokenizer(const std::string seq, const std::string separators, std::vector<std::string>* result) {
...
int usefulTools::NumberOfSetBits(uint32_t i){
    // Java: use >>> instead of >>
    // C or C++: use uint32_t
    // For some reason, this doesn't work in our case, I will need better knowledge to change this magic
    // should be fine as long as we use gcc
    i = i - ((i >> 1) & 0x55555555);
